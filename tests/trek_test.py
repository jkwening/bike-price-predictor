...
class SpecializedTestCase(unittest.TestCase):
    def setUp(self):
        self._trek = Trek(save_data_path=DATA_PATH)
    ...
    def test_get_categories(self):
        ...
        result = self._trek._get_categories(soup)
    ...
    def test_get_num_bikes(self):
        ...
        num = self._trek._get_max_num_prods(soup)
        expected = 268
    ...
    def test_get_prod_listings(self):
        ...
        with open(SHOP_BIKES_HTML_PATH, mode='r',
            ...
            soup = BeautifulSoup(html, 'lxml')
        self._trek._get_prods_on_current_listings_page(
            soup, bike_type='all')
        num_bikes = len(self._trek._products)
        self.assertEqual(num_bikes, expected,
                         msg=f'Expected: {expected}; parsed: {num_bikes}')

    def test_get_all_available_prods(self):
        # Validate method
        self._trek.get_all_available_prods()
        num_prods = len(self._trek._products)
    ...
    def test_parse_prod_spec(self):
        ...
        result = self._trek._parse_prod_specs(detail_soup1)
        ...
        result = self._trek._parse_prod_specs(detail_soup2)
        ...
        result = self._trek._parse_prod_specs(detail_soup3)
